#'
#' @title Create a \link{tibble}[tibble] representing a single time step and depth layer from a ROMS 4DVar in a netcdf file
#'
#' @description Function to create a \link{tibble}[tibble] representing a single time step and depth layer from a ROMS 4DVar in a netcdf file.
#'
...
#'
#' @return A \link{tibble}[tibble] object representing the layer
#'
#' @details Creates a \link{tibble}[tibble] representing a single time step and depth layer from a ROMS 4DVar in a netcdf file.
#'
#' @import ncdf4
#' @import wtsGIS
#' @import wtsUtilities
...
#' @export
#'
netCDF_Extract4DVarAsTbl<-function(netcdf,
                                  variable,
                                  timeslice=1,
                                  layer=1,
                                  verbose=FALSE){
  if (class(netcdf)!="ncdf4"){
    if (verbose) message("netCDF_Extract4DVarAsTbl: opening netcdf file '",netcdf,"'");
  ...
  if ((layer>vi[id_s])|(layer<1)) stop("netCDF_Extract4DVarAsTbl: requested layer does not exist.")
  ...
  dfr<-tibble::tibble(ID=kid,xi=kxi,eta=ket,var=kvr);
