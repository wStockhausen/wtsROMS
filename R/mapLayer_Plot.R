...
#' @param aes.palette - a list (with elements 'cat', 'div', 'seq') of palette names or a vectors of colors for the basemap.
...
mapLayer_Plot<-function(obj,
  ...
  if (inherits(obj,"tmap")) {
    ...
    layer<-wtsROMS::mapLayer_CreateFromModelOutput(obj,col=col,n=n,style=style,breaks=breaks,palette=palette);
    message("created map layer")
  }
  map <- basemap + layer;
  if (showMap) {message("--printing map"); print(map); message("--finished printing map");}
